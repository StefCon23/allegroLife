...
void initField(int xLength, int yLength, int zLength, 
	...
	point* cells, point* originalCells, bool* nextCells)
{
	/*int xlen = 90;
	int ylen = 90;
	int zlen = 90;
	...
	*zLengthFlag = zLength;
	...
	realloc(cells,xlen*ylen*zlen*sizeof(point));
	realloc(originalCells, xlen*ylen*zlen*sizeof(point));
	realloc(nextCells, xlen*ylen*zlen*sizeof(bool));
...
//////~~~~~~~~~~~~~~~~~~~~CODE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		*/
		...
		//rules
		textprintf(backBuffer, font, w - 550, h - 120, makecol(dotCol.x, dotCol.y, dotCol.z), "live cells with less than %d or more than %d neighbours dies", live1, live2);
		...
		textprintf(backBuffer, font, 20, h - 180, makecol(dotCol.x, dotCol.y, dotCol.z), "the board is:");
		textprintf(backBuffer, font, 20, h - 160, makecol(dotCol.x, dotCol.y, dotCol.z), "%d cells wide", xlen);
		textprintf(backBuffer, font, 20, h - 140, makecol(dotCol.x, dotCol.y, dotCol.z), "%d cells high", ylen);
		textprintf(backBuffer, font, 20, h - 120, makecol(dotCol.x, dotCol.y, dotCol.z), "%d cells deep", zlen);
